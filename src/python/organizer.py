...
def main():
    try:
        ...
        total_max_index = max(
            int(index.strip()) for indices in total_photo_indices for index in indices
        )
        ...
        if s:
            ...
            for i, row in data.iterrows():
                ...
                photo_index = int(row[5]) - 1
                photo_group = sample_photo_files[photo_index * 2 : photo_index * 2 + 2]

                convex_filename = f"{accuno}标{int(row[4])}凸面.JPG"
                concave_filename = f"{accuno}标{int(row[4])}凹面.JPG"
                ...
                if os.path.exists(convex_dest):
                    print(f"标本 {int(row[4])} 重复，请检查data.xlsx")
